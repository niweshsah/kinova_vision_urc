...
class YOLOv8ROSNode:
    def __init__(self):
        ...
        # Add a parameter for the label ID to filter detections
        self.label_id = rospy.get_param('~label_id', default=None)  # Default to None (detect all objects)
        rospy.loginfo(f"Filtering detections for label ID: {self.label_id}")
            ...
            if self.depth_image is not None:
                ...
                for result in results:
                    for box in result.boxes:
                        ...
                        if self.label_id is not None and class_id != self.label_id:
                            continue  # Skip detections that don't match the specified label ID
    ...
    def publish_and_display(self, rendered_image, depth_image):
        """Publish the processed image and display it for debugging."""
        try:
            image_msg = self.bridge.cv2_to_imgmsg(rendered_image, encoding="bgr8")
